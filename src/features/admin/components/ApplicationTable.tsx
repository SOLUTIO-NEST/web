...
export default function ApplicationTable({
    ...
    const getStatus = (isApprove: boolean | null) => {
        if (isApprove === true) return 'ACCEPTED';
        if (isApprove === false) return 'REJECTED';
        return 'PENDING';
    };
                ...
                <table className="w-full text-left text-sm">
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {applications.map((app) => {
                            const status = getStatus(app.isApprove);
                            return (
                                <tr key={app.studentId} className="hover:bg-gray-50/50 transition">
